...
export class Game {
    ...
    private violinRenderer: ViolinFingerboardRenderer;
    private instrumentMode: 'piano' | 'violin' = 'piano';
    ...
    constructor() {
        ...
        this.violinRenderer = new ViolinFingerboardRenderer('note-options');
        ...
        if (activeProfile) {
            ...
            // Load instrument mode from profile
            const savedInstrumentMode = this.profileManager.getActiveProfileInstrumentMode();
            this.setInstrumentMode(savedInstrumentMode);
        } else {
            // No active profile (anonymous user)
            // Load instrument mode from global storage
            const savedInstrumentMode = this.storageManager.loadInstrumentMode();
            this.setInstrumentMode(savedInstrumentMode);
        }
    ...
    public setActiveProfile(id: string): void {
        if (this.profileManager.setActiveProfile(id)) {
            ...
            if (profile) {
                ...
                // Load instrument mode
                const savedInstrumentMode = this.profileManager.getActiveProfileInstrumentMode();
                this.setInstrumentMode(savedInstrumentMode);
            } else {
                // Load instrument mode from global storage if no profile is active
                const savedInstrumentMode = this.storageManager.loadInstrumentMode();
                this.setInstrumentMode(savedInstrumentMode);
            }
    ...
    private loadLevel(levelIndex: number): void {
        ...
        // Apply instrument filter to the new level
        this.applyInstrumentFilter();
    ...
    private renderNoteOptions(): void {
        ...
        if (this.instrumentMode === 'piano') {
            ...
            // Use the violin fingerboard renderer
            this.violinRenderer.renderFingerboard(availableNotes, (selectedNote: Note) => {
                this.checkAnswer(selectedNote);
            });
        }
    }
    ...
    public setInstrumentMode(mode: 'piano' | 'violin'): void {
        this.instrumentMode = mode;

        // Save to profile if available
        if (this.profileManager.getActiveProfile()) {
            this.profileManager.updateActiveProfileInstrumentMode(mode);
        } else {
            // Save to global storage if no profile is active
            this.storageManager.saveInstrumentMode(mode);
        }
        ...
        // Notify UI of change
        document.dispatchEvent(new CustomEvent('instrumentModeChanged', { detail: { mode: this.instrumentMode } }));
    ...
    private applyInstrumentFilter(): void {
        if (!this.currentLevel) return;

        if (this.instrumentMode === 'violin') {
            this.currentLevel.setNoteFilter((note: Note) => {
                return this.violinRenderer.isNoteOnFingerboard(note);
            });
        } else {
            this.currentLevel.setNoteFilter(null);
        }
    }

    public getInstrumentMode(): 'piano' | 'violin' {
