...
export class ProfileManager {
    ...
    public createProfile(name: string): Profile {
        ...
        const newProfile: Profile = {
            ...
            instrumentMode: 'piano' // Default to piano
        };
    ...
    /**
     * Updates the instrument mode for the active profile
     */
    public updateActiveProfileInstrumentMode(mode: 'piano' | 'violin'): boolean {
        const activeProfile = this.getActiveProfile();
        ...
        activeProfile.instrumentMode = mode;
        activeProfile.lastUsed = Date.now();
        this.saveProfilesState();
        return true;
    }
    ...
    /**
     * Gets the instrument mode for the active profile
     */
    public getActiveProfileInstrumentMode(): 'piano' | 'violin' {
        const activeProfile = this.getActiveProfile();

        if (!activeProfile || !activeProfile.instrumentMode) {
            return 'piano'; // Default
        }

        return activeProfile.instrumentMode;
    }
} 